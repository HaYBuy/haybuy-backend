...
async def create_transaction(
    ...
    existing_seller = db.query(User).filter(User.id == data.seller_id).first()
    ...
    if existing_item.owner_id == current_user["id"]:
        raise HTTPException(
            status_code=403,
            detail="You cannot perform this action on your own item"
        )
...
@rounter.patch("/transaction/cancel/{transaction_id}", response_model=TransactionResponse)
def cancel_transaction(
    ...
    if transaction.status == TransactionStatus.ACCEPTED and transaction.buyer_id == user_id:
        raise HTTPException(
            status_code=403,
...
async def change_transaction_detail(
    ...
    if existing_transaction.seller_id != current_user["id"] and existing_transaction.buyer_id != current_user["id"] :
        raise HTTPException(status_code=403, detail="You are not the part of this transaction")
    
    if existing_transaction.status in [TransactionStatus.CANCELLED, TransactionStatus.ACCEPTED]:
        raise HTTPException(
            status_code=400,
...
def update_transaction_accept(
    ...
    if role == TransactionRole.buyer and transaction.buyer_id != user_id:
        raise HTTPException(status_code=403, detail="You are not the buyer of this transaction")
    elif role == TransactionRole.seller and transaction.seller_id != user_id:
        raise HTTPException(status_code=403, detail="You are not the seller of this transaction")
    ...
    if transaction.buyer_accept and transaction.seller_accept:
        ...
        if item_db.quantity>= transaction.amount:
            ...
            raise HTTPException(status_code=400, detail=f"Item is not available ,remaining :{item_db.quantity}")
    else:
