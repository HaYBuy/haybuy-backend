...
async def get_my_groups(
    ...
    groups = (
        db.query(Group)
        .filter(Group.owner_id == current_user["id"], Group.deleted_at == None)
        .all()
    )
    return groups
...
async def get_group_by_id(group_id: int, db: Session = Depends(get_db)):
    db_group = (
        db.query(Group).filter(Group.id == group_id, Group.deleted_at == None).first()
    )
    if not db_group:
...
async def get_all_groups(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    groups = (
        db.query(Group).filter(Group.deleted_at == None).offset(skip).limit(limit).all()
    )
    return groups
