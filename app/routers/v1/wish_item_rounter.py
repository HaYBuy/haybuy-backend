...
async def add_wish_item(
    ...
    existing_wish_item_db = db.query(WishItem).filter(WishItem.item_id == wish.item_id, WishItem.user_id == current_user["id"]).first()
...
async def remove_wish_item(
    ...
    db_wish_item = db.query(WishItem).filter(WishItem.id == wish_id, WishItem.user_id == current_user["id"]).first()
    if not db_wish_item:
...
async def get_my_wish_list(
    ...
    items = db.query(Item).join(WishItem, Item.id == WishItem.item_id)\
