...
async def add_member_to_group(
    ...
    existing_member = db.query(GroupMember).filter(GroupMember.group_id == group_id, GroupMember.user_id == member.user_id).first()
    if existing_member:
...
async def get_members_in_group_public(
    group_id: int,
    db: Session = Depends(get_db)
):
